...
#modified version of fm demod arctan
def secondaryFmDemodArctan(I, Q, prev_I = 0.0, prev_Q = 0.0):
    fm_demod = np.empty(len(I))
    for k in range(len(I)):
        current_I = I[k]
        current_Q = Q[k]
        if (current_Q != 0 and current_I != 0):
...
#fucntion for coefficient of band pass (pseudocode given project doc)
def pcoeffBPIR(Fb, Fe, Fs, num_taps):
    ...
    for i in range(num_taps):
        if i == (num_taps - 1) / 2:
            ...
            h[i] = normPass * (np.sin(math.pi * (normPass / 2) * (i - (num_taps - 1) / 2)) / (math.pi * (normPass / 2) * (i - (num_taps - 1) / 2)))
        h[i] = h[i] * np.cos(i * math.pi * normCenter)
        h[i] = h[i] * np.sin((i * math.pi) / num_taps) * np.sin((i * math.pi) / num_taps)
