...
int main()
{
  ...
  std::vector<std::complex<float>> Xf;
	...
	std::vector<float> psd_estimate;
	double Fs = 240;

	estimatePSD(bin_data, Fs, freq, psd_estimate);
	vector_index.clear();
	genIndexVector(vector_index, psd_estimate.size());
	logVector("demod_psd", freq, psd_estimate);
