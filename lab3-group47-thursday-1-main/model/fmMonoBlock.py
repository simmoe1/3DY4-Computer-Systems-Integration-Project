...
from blockProcessing import convolution
...
if __name__ == "__main__":
	...
	if il_vs_th == 0:
		...
		#audio_coeff = np.array([])
		audio_coeff = lowPassImpulse(audio_Fc, audio_Fs, audio_taps)
	...
	state_phase = 0
	zi_audio_filt = np.zeros(audio_taps-1)
	...
	while (block_count+1)*block_size < len(iq_data):
		...
		#		zi=state_q_lpf_100k)
		i_filt, state_i_lpf_100k = convolution(rf_coeff, iq_data[(block_count)*block_size:(block_count+1)*block_size:2], 1.0, state_i_lpf_100k)
		q_filt, state_i_lpf_100k = convolution(rf_coeff, iq_data[(block_count)*block_size:(block_count+1)*block_size:2], 1.0, state_i_lpf_100k)
		...
		# 	# same principle as for i_filt or q_filt (but different arguments)
			audio_filt, zi_audio_filt = signal.lfilter(audio_coeff, 1.0, fm_demod, zi=zi_audio_filt)
		else:
		# 	# to be updated by you for the takehome exercise
		# 	# with your own code for BLOCK convolution
			audio_filt, zi_audio_filt = convolution(audio_coeff, fm_demod,1.0, zi=zi_audio_filt)
		...
		if block_count >= 10 and block_count < 12:
			...
			fmPlotPSD(ax0, audio_filt, (rf_Fs/rf_decim)/1e3, subfig_height[0], \
					'Extracted Mono Audio FM ' + str(block_count) + ' ')
			# ... change as needed

			# plot PSD of selected block after downsampling mono audio
			fmPlotPSD(ax0, audio_block, (rf_Fs/rf_decim)/1e3, subfig_height[0], \
					'Downsampled Mono Audio ' + str(block_count) + ' ')
			# ... change as needed
	...
	fm_demod.astype('float32').tofile("../data/fm_demod_10.bin")
