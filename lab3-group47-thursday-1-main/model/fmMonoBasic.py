...
if __name__ == "__main__":
	...
	rf_coeff = lowPassImpulse(rf_Fc, rf_Fs, rf_taps)

	# filter to extract the FM channel (I samples are even, Q samples are odd)
	i_filt = signal.lfilter(rf_coeff, 1.0, iq_data[0::2])
	q_filt = signal.lfilter(rf_coeff, 1.0, iq_data[1::2])
	#ziDummy = np.zeros(1)
	#i_filt = convolution(rf_coeff, 1.0, iq_data[0::2], ziDummy)
	#q_filt = convolution(rf_coeff, 1.0, iq_data[1::2], ziDummy)
	...
	if il_vs_th == 0:
		...
		#audio_filt = np.array([])
		audio_filt = convolution(audio_coeff, 1.0, fm_demod)
